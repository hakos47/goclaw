...
var coreToolSummaries = map[string]string{
	...
	"whatsapp_list_chats":   "List WhatsApp conversations. Use parameter type='all' (default) to see both groups and direct 1-to-1 chats, or type='direct' for only private chats.",
	"whatsapp_list_contacts":"List all WhatsApp contacts stored by the linked account",
	"whatsapp_send_message": "Send a text message to a WhatsApp user or group via JID",
	"whatsapp_group_create": "Create a new WhatsApp group with specified participants",
	"whatsapp_group_invite": "Get the WhatsApp invite link for a group",
	"whatsapp_group_members": "List all members of a WhatsApp group",
	"whatsapp_profile_photo": "Get the profile photo URL for a WhatsApp user or group",
	"whatsapp_find_contact":  "Find a WhatsApp contact by name or partial name. Searches GoClaw's contact database first, then falls back to WhatsApp local contacts",
	"whatsapp_get_profile":   "Get comprehensive profile information for a JID (push name, bio, business info).",
	"whatsapp_get_status":    "Get the current bio/status text of a WhatsApp user JID.",
	"whatsapp_read_messages": "Read the recent message history of a WhatsApp conversation (direct or group).",
	"whatsapp_test_target":   "Run a complete diagnostic test against a phone number (profile info, photo, and bio).",	"delegate":               "Delegate a task to a linked agent (requires agent_links). See ## Delegation Targets for available agents",
