-- 000057_robust_rls.up.sql
-- Fix RLS policy to be more robust for Go application pool usage.
-- Allows access if the tenant context is set OR if we are in a system bypass mode (setting empty).
...
BEGIN
    ...
    LOOP
        ...
        -- New policy: tenant must match context. If context is empty, it's a system bypass (internal maintenance).
